...
const OverviewCards = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mt-30">
      ...
      <OverviewCard Icon= {<ArrowTrendingUpIcon className="h-6 w-6 text-purple-500"/>} count= {Math.floor(100*stats.purchases/stats.views)} label= "Conversion rate" color={'bg-purple-100'} />
      <OverviewCard Icon= {<ArrowTrendingDownIcon className="h-6 w-6 text-yellow-500"/>} count= {Math.floor(100*stats.abandonments/stats.views)} label= "Abandonment rate" color={'bg-yellow-100'} />
    </div>
  );
};
